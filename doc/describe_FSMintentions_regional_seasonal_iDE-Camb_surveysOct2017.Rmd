...
# summary(d.sets.prov, maxsum = 500)
d.sets.prov = subset(d.sets.prov, 
                     select = -c(Pop13M, Pop13F, Pop13MPerc, 
                          PopChng08to13MPerc,
                          PopChng08to13FPerc,
                          PopGrow08to13Perc,
                          MedAge13M, MedAge13F,
...
# print(prop.col[,1:length(prop.col[1,])])

df = data.frame(Test5 = prop.col)

df = data.frame(temp = prop.col)
...
d = subset(d, select = -c(Date, LBO, Dist, Comm, Vill, IDPoorTyp,
                          ...
                          IntndChng_Sink, IntndChng_WtrRes,
                          IntndChng_Pit, IntndChng_Othr, 
                          IntndChng_NAAlwysToi, IntndPitFull,
                          IntndPitFullPit, IntndPitFullEmpSlf,
                          IntndPitFullDK, IntndPitFullOthr,
                          IntndPitFullPay, IntndPitFullStop,
                          Pop13, Pop13M, Pop13F, PopChng08to13,
                          ...
                          MigrntIn, MigrntOut, MigrntNet,
                          AreaKm2))
# not removed: IntndPitFullDes, Satis, SatisSup
# is removed: IntndPitFull, SatisColapsMore, SatisSupColapsMore
d = subset(d, select = -c(IDPoorHHpercDist, IDPoorHHpercComm,
                          IDPoor1HHpercDist, IDPoor2HHpercDist, 
                          IDPoor1HHpercComm, IDPoor2HHpercComm, 
                          IDPoor1HHpercVill, IDPoor2HHpercVill))
...
indep_vars = subset(data.frame(vars = names(d)),
                    vars != dep_var)
formula = paste(dep_var, " ~ ", paste(indep_vars$vars, collapse = " + "), sep = "")
# Formula used in below model:
# IntndPitFullDes ~ Prov + IDPoor + LivRP + VillOD + WhoInstLat + RecSubsdy + WallMat + RoofMat + Rec + RecSup + Yr + Mnth + RDefBefor_BshFld + ChlngsNoFlsh + ChlngsFlood + ChlngsOthr + ChlngsFulOvrFlw + ChlngsNoWtr + ChlngsSmels + ChlngsOK + SatisColaps + SatisSupColaps + Pop13FPerc + PopChng08to13Perc + PopDens13 + PopDensChng08to13 + MigrntNetPercPop + GndrRatio014 + GndrRatio1564 + GndrRatio65up + MedAge13 + Pop014Perc + Pop1549Perc + Pop5064Perc + Pop65upPerc + MaridNevr2024DiffMF + MaridF + WidwdF + Widwd60upDiffMF + DvrcdSepM + DvrcdSepF + DvrcdSep60upF + MeanAgeMaridF + MeanAgeMaridDiffMF + DisabPerc + DisabMPerc + DisabMedAge + DisabMedAgeM + DisabMedAgeF + DisabEconAct1564Perc + NatvLng_Khmer + GenLitRat13 + GenLitRatChng08to13 + GenLitRatChng08to13F + GenLitRatDiffChng08to13MF + EducNone + EducPrimIncomp + EducPrimComp + EducNoneNPrimIncompM + EducNoneNPrimIncompF + EducPrimCompM + EducPrimCompF + SchlAttnd13M + SchlAttnd13F + SchlAttndChng08to13 + SchlAttndDiffChng08to13MF + SchlAttndPerc5dwn + SchlAttndPerc614 + SchlAttndPerc1519 + SchlAttndPerc2024 + SchlAttndPerc25up + FertRat13 + EconAct15up13 + EconAct15upM13 + EconAct15upF13 + EconAct15upChng08to13 + EconAct15upMChng08to13 + EconAct15upFChng08to13 + JobEmplyr13 + JobEmplye13 + JobSlfEmplyd13 + JobUnpaidFamWrk13 + JobSectrGov13 + JobSectrStateEntrprs13 + JobSectrCambPrivEntrprs13 + JobSectrForgnEntrprs13 + JobSectrNonProfOthr13 + JobIndstryPri13 + JobIndstrySec13 + JobAgForFsh13 + JobManufctr13 + JobWhlslRetlTradOthr13 + JobManagr + JobProfsnl + JobTechAssisProfsnl + JobClericl + JobServcSals + JobCraft + JobMachOprtrs + JobElementry + JobMilitry + JobSecndJobPerc + JobChld514Perc + JobChld514MPerc + JobChld514FPerc + JobUnemply1524Perc + JobUnemply1524MPerc + JobUnemply1524FPerc + BldngsPermRoofPerc + BldngsPermRoofwallFlorPerc + HshldsAvgNumPpl + HshldsAvgNumPplPerRoom + HshldsLightElecPerc + HshldsLightKeroPerc + HshldsLightBattPerc + HshldsCookFuelWoodPerc + HshldsCookFuelCharPerc + HshldsCookFuelLPGPerc + HshldsDrnkWtrPipedPerc + HshldsDrnkWtrTubPipWelPerc + HshldsDrnkWtrProtDugWelPerc + HshldsDrnkWtrUnprotDugWelPerc + HshldsDrnkWtrRainPerc + HshldsDrnkWtrSprngRivrPerc + HshldsDrnkWtrPurchsdPerc + HshldsDrnkWtrOthrPerc + HshldsDrnkWtrOnSitePerc + HshldsDrnkWtrNearPerc + HshldsDrnkWtrAwayPerc + HshldsToi13Perc + HshldsToiChng08to13Perc + HshldsToiSewer13Perc + HshldsToiSeptc13Perc + HshldsToiLatrn13Perc + HshldsImprvdWtr + HshldsImprvdToi + HshldsElec + HshldsImprvdWtrToiElec + PopAnulGrowRate + FHeadHHPerc + NuclrFamPerc + IDPoorPopPerc + LatCovPerc + IDPoorHHpercVill
