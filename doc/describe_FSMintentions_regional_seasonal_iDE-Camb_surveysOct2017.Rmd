...
The data analyzed with multiple correspondence analysis is first summarized by frequency using bar charts of each variable.

```{r MCA_summarize, echo = FALSE}
par(mfrow = c(3,1))
for (i in 1:length(data.mca)) {
  if (i %% 3 == 0) {par(mfrow = c(3,1))}
  plot(data.mca[,i], main = colnames(data.mca)[i],
       ylab = "Count", col = "steelblue", las = 2)
}
```
